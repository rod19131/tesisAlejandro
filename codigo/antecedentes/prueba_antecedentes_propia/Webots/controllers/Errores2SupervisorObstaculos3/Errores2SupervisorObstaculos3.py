"""=========================================================================
# MEDICIÓN DE MÉTRICAS DE ENERGÍA Y CAMBIOS DE FORMACIÓN EN LA SIMULACIÓN
# DEL MODELO DINÁMICO CON CONTROL DE FORMACIÓN, USANDO COSENO HIPERBÓLICO, 
# Y EVASIÓN DE OBSTÁCULOS INCLUYENDO LÍMITES DE VELOCIDAD
# =========================================================================
...
"""Errores2SupervisorObstaculos3 controller."""
...
workbook = xlsxwriter.Workbook('ResultadosWebots2.xlsx')
...
EI = []			# error inicial de la formación en cada simulación
EIF = []			# error final de la formación en cada simulación					
ER = []			# error relativo de la formación en cada simulación
...
cambiosForm = []	# cambios de formación por simulación
...
for I in range(0,cantI):
    ...
    while (supervisor.step(TIME_STEP) != -1 and errorDist > 0.1 and ciclos < 3000):
        ...
        if(cambio == 2 and cambio_old == 1):
            cambio_old = 2
            mDistF1 = 5*funciones.DistEntreAgentes(posActuales)
            errorF1 = funciones.ErrorForm(mDistF1, Fmatrix(FSel,8))
    ...
    mDistF2 = 5*funciones.DistEntreAgentes(posActuales)
    errorF2 = funciones.ErrorForm(mDistF2, Fmatrix(FSel,8))
    
    cambiosForm.append(cantCambios)		# total de cambios de formación
    
    if(cambio == 2):
        EI.append(errorF1)
        EIF.append(errorF2)
        ER.append(errorF2/errorF1)
    else:
        fallos = fallos + 1
...
print("EIF",EIF)
print("ER",ER)
print("cambiosForm",cambiosForm)
print("fallos",fallos)
print("cantMeta",cantMeta)

# Grabado de datos en Excel
worksheet1.write_row('B3',EI)
worksheet1.write_row('B5',EIF)
worksheet1.write_row('B7',ER)
worksheet1.write_row('B9',cambiosForm)
worksheet1.write('B11',fallos)
worksheet1.write('B13',cantMeta)
workbook.close()
